...
void RayTracer::render(const Camera &camera, const HittableList &world,
...
void RayTracer::renderMultiThread(const Camera &camera,
  ...
  const size_t rows_per_thread =
      std::max(image_height / max_num_threads, kMinNumRowsPerThread);
  ...
  for (size_t i = 0; i < kNumThreads; ++i) {
    const size_t num_rows =
        (i == kNumThreads - 1)
            ? (image_height - rows_per_thread * (kNumThreads - 1))
            : rows_per_thread;
    threads.emplace_back(&RayTracer::threadRendering, this, &camera, &world,
                         curr_row, num_rows, &row_buffers, i);
    thread_guards.push_back(new ThreadGuard(threads.back()));
    curr_row += num_rows;
  }
  threads.emplace_back(&RayTracer::threadPrintingProgressBar, this,
                       image_height, image_width, &row_buffers);
  thread_guards.push_back(new ThreadGuard(threads.back()));
